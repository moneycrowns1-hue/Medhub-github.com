...
export function AiGeneratorStub({ request, disabled }: Props) {
  ...
  return (
    <div className="rounded-xl bg-purple-400/10 p-3 text-xs text-purple-100">
      ...
      {result && result.status === "disabled" ? (
        <div className="mt-2 rounded-md bg-amber-400/10 px-2.5 py-1.5 text-amber-100">
          {result.message}
        </div>
      ) : null}
      {result && result.status === "ok" ? (
        <div className="mt-2 rounded-md bg-emerald-400/10 px-2.5 py-1.5 text-emerald-100">
